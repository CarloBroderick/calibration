...
```{r}
monthly_nse = function(m, o, month) {
  ...
  # Calculate the monthly nse metric
...
```{r}
month_m <- function(m,o) {
  mean_month = sager|>
    group_by(month)|>
    summarise(mean_model = mean(model), mean_obs = mean(obs))
  month_error = sum(mean_month$mean_model - mean_month$mean_obs)
  month<- abs(month_error)
  month_final <- exp(-month)
  return(month_final)
}
```

```{r}
kge = function(m, o, month) {
  # Create a dataframe to get the averages for the modeled values and the observation values
  df = data.frame(m, o, month) |>
    group_by(month) |>
    summarise(ave_m = mean(m), ave_o = mean(o), 
...
combined_performance_metric = function(m, o, month) {
  ...
  # Calculate the KGE (Kling-Gupta Efficiency) metric for each month
  beta = df$ave_m/df$ave_o
  alpha = df$sd_m/df$sd_o
  r = cor(m, o, method = "pearson")
  kge = 1.0 - sqrt(((r - 1)^2) + ((alpha - 1)^2) + ((beta - 1)^2))
  kge_norm = exp(-abs(kge))
...
```{r}
# Test out the combined_performance_metric function on our data
combined_performance_metric(m = sager$model,o = sager$obs, month = sager$month)
```

```{r}
kge(m = sager$model, o = sager$obs,  month = sager$month)
monthly_nse(m = sager$model, o = sager$obs,  month = sager$month)
month_m(m = sager$model, o = sager$obs)
```
...
```{r}
# subset for split sample calibration
test_split <- subset(msage, wy < 1970)
  
train_split = subset(msage, wy > 1970)
```

```{r}
res = train_split %>% select(!c("date","month","year","day","wy","obs")) %>%
      map_dbl(combined_performance_metric, month = train_split$month, train_split$obs)
```
