...
A**s a group we wanted to see if the model was getting things right on average but we thought that an annual metric would loose some of the seasonal variability, so we created two indicators that measure monthly average model performance and combined them.**

```{r}
# create custom performance metric
combined_performance_metric = function(m, o, month) {

  # create a data frame and calculate monthly mean for m and o
  df = data.frame(m, o, month) |>
    group_by(month) |>
    summarise(ave_m = mean(m), ave_o = mean(o))
  
  # create a monthly average NSE score
  ...
  # create a monthly error score and normalize
  month_error = sum(df$ave_m - df$ave_o)
  month<- abs(month_error)
  month_error <- exp(-month)
  
  # simple multiplication of perfomance metrics
  combined = monthly_nse * month_error

  # return combined performance metric
  return(combined)
}
...
res = msage %>% select(!c("date","month",
                          "year","day","wy",
                          "obs")) %>%
  map_dbl(combined_performance_metric, month = msage$month, msage$obs)
```

```{r}
# make the output of the map funciton a df
res <- as.data.frame(res)

# put the names of the models on the data frame
res$sim = snames

# get the best and word perfomring models 
